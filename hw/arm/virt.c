...
void virt_machine_done(Notifier *notifier, void *data)
{
    ...
    /*
     * Skip fw_cfg extra PCI roots when pci-pre-enum is on: with EDK2 bus
     * enumeration disabled, each fw_cfg-reported root tries to claim the same
     * host MMIO/IO windows in GCD and the firmware asserts on the second root.
     */
    if (!vms->pci_pre_enum) {
